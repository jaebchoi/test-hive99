            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "test-hive99 authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "test-hive99 failed to authenticate properly"
                throw err
            }
        }
    }
        ...
        try {
            dir(projectDirectory) {
                ...
                slackSend color: "good",
                        message: "test-hive99 deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "test-hive99 failed to deploy properly"
            throw err
        }
    }
}
...
stage("Teardown") {
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "test-hive99 shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "test-hive99 failed to shut down properly"
            throw err
        }
    }
}
